...
La probabilidad de seleccionar la acción greedy con dos acciones es

$$ 1-\epsilon + \frac{\epsilon}{2}  = 1-0.1 + \frac{0.1}{2} = 0.95$$

puesto que al explorar debemos elegir entre las dos acciones con igual probabilidad, 
entonces dividimos la probabilidad de exploración por 2.
...
y la derivada de $E[R_t]$ respecto a $H_t(\alpha)$ es:

aunque hay que retener que $q_*(x)$ es deconocido.

Ahora calculamos la derivada de $E[R_t]$ respecto a $H_t(\alpha)$:

$$ \frac{\partial E[R_t]}{\partial H_t(\alpha)} = \sum_{x=1}^{K} q_*(x) \frac{\partial p_t(x)}{\partial H_t(\alpha)} $$

A esta expresión le podemos aplicar una constante $C$, 
puesto que $\sum_x \frac{\partial p_t(x)}{\partial H_t(\alpha)} = 0$, dado que 
$\sum_x p_t(x) = 1$, por lo tanto no afecta al gradiente.

$$ = \sum_x [q_* -C] \frac{\partial p_t(x)}{\partial H_t(\alpha)} $$

luego multiplicamos por $p_t(x)$ y dividimos por $p_t(x)$:

$$ = \sum_x [q_* -C] \frac{\partial p_t(x)}{\partial H_t(\alpha)} \cdot \frac{p_t(x)}{p_t(x)} $$

vamos a asumir que $x=A_t$, entonces podemos reescribir la expresión como:

$$ = E[(q_* -C) \frac{1}{p_t(A_t)} \frac{\partial p_t(A_t)}{\partial H_t(\alpha)}] $$

también vamos a asumir que $q_* = E[R_t | A_t] = R_t$, en una suerte de stochastic gradient ascent con una sola observación:

$$ = E[(R_t -C) \frac{1}{p_t(A_t)} \frac{\partial p_t(A_t)}{\partial H_t(\alpha)}] $$

finalmente, usamos el resultado del ejercicio anterior para reemplazar la derivada de $p_t(A_t)$:

$$ = 
\begin{cases}
E[p_t(A_t)(1 - p_t(A_t)) \frac{1}{p_t(A_t)}] & \text{ si } A_t = \alpha \\
E[-p_t(A_t)p_t(\alpha) \frac{1}{p_t(A_t)}] & \text{ si } A_t \neq \alpha
\end{cases} $$

simplificamos en ambos casos:

$$ = 
\begin{cases}
E[(R_t -C)(1 - p_t(\alpha))] & \text{ si } A_t = \alpha \\
-E[(R_t -C)p_t(\alpha)] & \text{ si } A_t \neq \alpha
\end{cases} $$

entonces, la regla de actualización queda (remplazando el valor esperado por la evaluación en el instante $t$):
$$ H_{t+1}(\alpha) =
\begin{cases} 
    H_t(\alpha) + \alpha (R_t - C)(1 - p_t(\alpha)) & \text{ si } A_t = \alpha \\
    H_t(\alpha) - \alpha (R_t - C)p_t(\alpha) & \text{ si } A_t \neq \alpha \\
\end{cases} $$

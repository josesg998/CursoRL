...
donde:
\begin{itemize}
    \item Con probabilidad $p = 0.9$: transición a terminal, $R=1$, $V(S_T) = 0$
    \item Con probabilidad $1-p = 0.1$: transición a sí mismo, $R=1$, $V(S_0)$
\end{itemize}

La ecuación queda:
$$V(S_0) = p \cdot (1 + \gamma \cdot 0) + (1-p) \cdot (1 + \gamma \cdot V(S_0))$$
$$= p \cdot 1 + (1-p) \cdot (1 + 1 \cdot V(S_0))$$
$$= p + (1-p) + (1-p)V(S_0)$$
...
Para $p = 0.9$:
$$V_{\text{teórico}}(S_0) = \frac{1}{0.9} = \frac{10}{9} \approx 1.111$$

En conclusión, con una sola visita la estimación Monte Carlo es muy sesgada, mientras
que con varias visitas se acerca más al valor teórico, pero sigue siendo una
estimación alta. Esto se debe a que con $p=0.9$, el agente tiende a permanecer
mucho tiempo en el estado no-terminal y acumula muchas recompensas antes de
transicionar al estado terminal. % TODO CHEQUEAR
...
Elijo unos codifienctes definidos como:

...
class Jugador:
    def __init__(self, dinero: int = PHI,W_nor_pos=W_NOR_POS,W_nor_neg=W_NOR_NEG,W_emp_pos=W_EMP_POS,W_emp_neg=W_EMP_NEG):
        ...
        self.suscept_emp        = np.random.uniform(0,0.5)     # susceptibilidad a estímulos empíricos (receptor)
        self.suscept_norm       = 0.5 - self.suscept_emp # susceptibilidad a estímulos normativos (dictador)
    ...
    def actualizar_aspiracion_normativa(self,dictadores:list):
        ...
        if self.estimulo_normativo>=0:
            self.suscept_norm*= (1 + self.W_nor_pos * self.estimulo_normativo)
            self.corregir_susceptibilidades()
            self.aspiracion_alfa+=(self.dinero-self.aspiracion_alfa)*self.suscept_norm * self.estimulo_normativo
        else:
            self.suscept_norm*= (1 + self.W_nor_neg * self.estimulo_normativo)
            self.corregir_susceptibilidades()
            self.aspiracion_alfa+=self.aspiracion_alfa*self.suscept_norm * self.estimulo_normativo
    ...
    def actualizar_aspiracion_empirica(self):
        ...
        if self.estimulo_empirico>=0:
            self.suscept_emp*= (1 + self.W_emp_pos * self.estimulo_empirico)
            self.corregir_susceptibilidades()
            self.aspiracion_alfa+=(self.dinero-self.aspiracion_alfa)*self.suscept_emp * self.estimulo_empirico
        else:
            self.suscept_emp*= (1 + self.W_emp_neg * self.estimulo_empirico)
            self.corregir_susceptibilidades()
            self.aspiracion_alfa+=(self.aspiracion_alfa)*self.suscept_emp * self.estimulo_empirico
    ...
    def corregir_susceptibilidades(self):
        ...
        self.suscept_norm = np.maximum(0, self.suscept_norm)
        self.suscept_emp = np.maximum(0, self.suscept_emp)

        # Rule 2: Handle the sum constraint.
        total_suscept = self.suscept_norm + self.suscept_emp
        ...
        if total_suscept > 1:
            ...
            # this is safe.
            self.suscept_norm = self.suscept_norm / total_suscept
            self.suscept_emp = self.suscept_emp / total_suscept

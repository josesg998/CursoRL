...
En la imagen \ref{fig:epsilon-greedy} se muestran los resultados del algoritmo $\epsilon$-greedy.

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{bandits.png}
    \caption{Resultados del algoritmo $\epsilon$-greedy con $\sigma=0,1, 10$}
    \label{fig:epsilon-greedy}
\end{figure}

\begin{itemize}
\item Con sigma = 0, el algoritmo con epsilon igual a cero va quedarse siempre en la primera 
acción que elija, por lo que no podrá explorar otras acciones. Por otra parte, con epsilon igual
a 0.01 y 0.1 el algoritmo podrá explorar otras acciones y encontrar la mejor política, siendo el caso
con epsilon igual a 0.1 el que más rápido converga a la mejor política.

\item Con sigma = 1 en epsilon = 0 al principio consigue una mejora en su recompensa promedio,
pero rápidamente vuelve a estancarse, mientras que con el resto de epsilons el comportamiento
es similar al gráfico anterior con sigma = 0, pero con una varianza mayor.
\item Con sigma = 10, la dispersión en las recompensas es tan grande que es irrelevante
la elección del valor de epsilon.
\end{itemize}

Como conclusión, podemos inferir que el valor de la varianza influye en la capacidad
del algoritmo para decidir el mejor curso de acción, pueso que la dispersión es tan
alta entre máquinas, que las distribuciones de las recompensas de las máquinas
se solapan ampliamente, haciendo casi indistinguible la ventaja entre elegir una 
u otra.
...
Sabemos que el esperado de la recompensa es:
...
Además, de la demostración anterior, sabemos que:
...
Distribuimos $q_*(\alpha)$ y $q_*(\alpha)p_t(\alpha)$:
...
Tomamos $p_t(\alpha)$ como factor común:

$$= p_t(\alpha)(q_*(\alpha) - E[R_t])$$

\subsubsection{Construcción de estimadores no sesgados}
...
Por tanto, necesitamos \textbf{construir un estimador no sesgado} del gradiente usando solo esta información limitada.

Del paso anterior obtuvimos:
$$\frac{\partial E[R_t]}{\partial H_t(\alpha)} = p_t(\alpha)(q_*(\alpha) - E[R_t])$$

Para construir estimadores no sesgados, necesitamos que $E[\text{estimador}] = \text{valor real}$.

\textbf{Caso 1: $\alpha = A_t$ (acción seleccionada)}

Cuando seleccionamos la acción $\alpha$, observamos $R_t$ que es una muestra de $q_*(\alpha)$, es decir:
$E[R_t | A_t = \alpha] = q_*(\alpha)$

Para obtener un estimador no sesgado del gradiente, aplicamos la identidad fundamental de \textbf{policy gradients}:

$E\left[\frac{R_t - C}{p_t(A_t)} \cdot \frac{\partial p_t(A_t)}{\partial H_t(\alpha)}\right] = \frac{\partial E[R_t]}{\partial H_t(\alpha)}$

Cuando $A_t = \alpha$:
\begin{itemize}
\item $\frac{\partial p_t(\alpha)}{\partial H_t(\alpha)} = p_t(\alpha)(1 - p_t(\alpha))$
\item Por tanto: $\frac{R_t - C}{p_t(\alpha)} \cdot p_t(\alpha)(1 - p_t(\alpha)) = (R_t - C)(1 - p_t(\alpha))$
\end{itemize}

Nuestro estimador es:
$\widehat{\nabla}_\alpha = (R_t - C)(1 - p_t(\alpha))$

\textbf{Caso 2: $\alpha \neq A_t$ (acción no seleccionada)}

Cuando no seleccionamos la acción $\alpha$, no observamos $q_*(\alpha)$ directamente. Sin embargo, podemos construir un estimador no sesgado usando la restricción fundamental de las probabilidades.

Dado que $\sum_{x=1}^K p_t(x) = 1$, tenemos que:
$\sum_{x=1}^K \frac{\partial p_t(x)}{\partial H_t(\alpha)} = 0$

Esto significa que los estimadores deben sumar cero. Como el estimador para la acción seleccionada es $(R_t - C)(1 - p_t(A_t))$, los estimadores para las acciones no seleccionadas deben compensar:

$\widehat{\nabla}_\alpha = -(R_t - C)p_t(\alpha)$

\subsubsection{Introducción del baseline}

Como $E[R_t]$ es desconocido, lo reemplazamos por un baseline $C$:
\begin{itemize}
    ...
    \item El baseline reduce la varianza del estimador sin afectar el valor esperado
\end{itemize}

\subsubsection{La Regla de Actualización Final}

Aplicando $H_{t+1}(\alpha) = H_t(\alpha) + \alpha \cdot \widehat{\nabla}_\alpha$:

\textbf{Si $\alpha = A_t$ (acción seleccionada)}:
$$H_{t+1}(\alpha) = H_t(\alpha) + \alpha(R_t - C)(1 - p_t(\alpha))$$

\textbf{Si $\alpha \neq A_t$ (acción no seleccionada)}:
$$H_{t+1}(\alpha) = H_t(\alpha) - \alpha(R_t - C)p_t(\alpha)$$
